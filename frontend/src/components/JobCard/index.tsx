...
import { getJobCity, isNewJob, type Job } from "../../data/jobs";
...
const JobCard = ({ job }: JobCardProps) => {
  ...
  const meta = [
    { icon: FiMapPin, label: city.label.replace(" - ", ", ") },
  ...
  return (
    <article className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-4 md:flex-row md:items-center md:justify-between md:p-5">
      ...
      <div className="flex gap-2 md:w-40 md:shrink-0 md:flex-col">
        ...
        <button
          type="button"
          className="flex-1 cursor-pointer rounded-md bg-blue-900 px-4 py-2 text-sm font-medium text-white transition-all duration-300 hover:bg-blue-900/80"
        >
          Candidatar-se
        </button>
      </div>
